...
def minimax(depth,tilesInfo,alpha,beta,player,secLastX,secLastY,lastX,lastY,initcall):
    ...
    if(depth==0 or len(moves)==0):
        ...
        if(initcall):
            return countPoints(tilesInfo)+depth/100,0
        else:
            return countPoints(tilesInfo)+depth/100
        ...
        if(player=="max"):            
            ...
            for move in moves:
                ...
                evalu=minimax(depth-1,tilesInfo,alpha,beta,"min",lastX,lastY,move[0],move[1],False)
                # undo move
                undoMove(move)
            ...
            for move in moves:
                # make move and evaluate by calling recursively
                makeMove(player,move)
                evalu=minimax(depth-1,tilesInfo,alpha,beta,"max",lastX,lastY,move[0],move[1],False)
                # undo move
                undoMove(move)
                # update bestmove and best evaluation
                if(evalu<minEval):
...
while len(possibleMoves(secLastX,secLastY,lastX,lastY))>0:
    ...
    depth=startdepth
    while(delayRunning):
        tempevalu,tempmove = minimax(depth,tilesInfo,-1000,1000,"max",secLastX,secLastY,lastX,lastY,True)
        if(not tempmove==0):
            evalu, move = tempevalu, tempmove
            print(delayRunning)
        depth+=1
    #output
    print("reached depth "+str(depth-1))
    ...
    if(assist):
        ...
        depth=startdepth
        while(delayRunning):
            tempevalu,tempmove = minimax(depth,tilesInfo,-1000,1000,"min",secLastX,secLastY,lastX,lastY,True)
            if(not tempmove==0):
                evalu, move = tempevalu, tempmove
            depth+=1
